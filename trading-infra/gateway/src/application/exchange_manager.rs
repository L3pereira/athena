...
use crate::domain::{ExchangeId, OrderBookWriter, WsEvent};
use crate::infrastructure::{RestClient, WsClient};
    ...
    B: OrderBookWriter + Clone + 'static,
{
    ...
    async fn start_exchange(
        ...
        market_data_config: crate::config::MarketDataConfigJson,
        symbols: Vec<String>,
...
mod tests {
    ...
    use crate::domain::{QualifiedSymbol, SnapshotWriter, StreamData, UpdateWriter};

    /// Mock order book writer for testing (gateway doesn't build real order books)
    ///
    /// Implements SnapshotWriter + UpdateWriter separately (ISP compliance).
    /// The blanket impl automatically provides OrderBookWriter.
    #[derive(Clone)]
    struct MockOrderBookWriter;

    impl SnapshotWriter for MockOrderBookWriter {
        fn apply_snapshot(
            &self,
            _key: &QualifiedSymbol,
            _snapshot: &trading_core::DepthSnapshotEvent,
        ) {
        }
    }

    impl UpdateWriter for MockOrderBookWriter {
        fn apply_update(&self, _exchange_id: &ExchangeId, _update: &StreamData) -> bool {
            true
        }
    }
    ...
    fn test_exchange_manager_creation() {
        ...
        let order_books = MockOrderBookWriter;
