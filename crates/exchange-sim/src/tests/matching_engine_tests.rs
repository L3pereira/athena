...
use athena_matching::{MatchingAlgorithm, PriceTimeMatchingEngine, ProRataMatchingEngine};

use crate::error::Result;
use crate::model::{InstrumentId, Order, OrderType, Side, TimeInForce};
...
fn test_price_time_matching_can_match() -> Result<()> {
    ...
    // Different instrument - should not match
    let mut different_symbol_sell = sell_order.clone();
    different_symbol_sell.instrument_id = InstrumentId::new("OTHER/USD");
