...
impl StreamData {
    /// Parse stream data using the default parsers
    pub fn from_ws_response(stream: &str, data: &Value) -> Option<Self> {
        use crate::gateway_in::infrastructure::{DepthParser, TradeParser};

        let parsers: &[&dyn StreamParser] = &[&DepthParser, &TradeParser];
        Self::from_ws_response_with_parsers(stream, data, parsers)
    }

    /// Parse stream data using a custom set of parsers (Open/Closed principle)
    pub fn from_ws_response_with_parsers(
        stream: &str,
        data: &Value,
        parsers: &[&dyn StreamParser],
    ) -> Option<Self> {
